...
export default function ExpensesPage() {
  ...
  // Initial load. State is set inside .then (async), not synchronously here.
  useEffect(() => {
    let active = true;
    api<Expense[]>("/expenses?scope=GENERAL")
      .then((rows) => {
  ...
  const total = useMemo(
    () => expenses.reduce((s, e) => s + e.amountKopiyky, 0),
    [expenses],
  );
  ...
  async function add(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(
        "Витрату додано",
        `${EXPENSE_KIND_LABEL[kind]} · ${formatUAH(uahToKopiyky(amount))}`,
      );
      await load();
    } catch (err) {
              ...
              <ul className="divide-y divide-line">
                {expenses.map((e) => (
                  <li key={e.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="nums shrink-0 font-semibold text-ink">
              ...
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody className="divide-y divide-line">
                  {expenses.map((e) => (
                    ...
                    >
                      <td className="nums px-5 py-3 text-ink-soft">
                        {e.incurredOn}
                      </td>
                      ...
                      <td className="nums px-5 py-3 text-right font-medium text-ink">
                        {formatUAH(e.amountKopiyky)}
                      </td>
                ...
                <tfoot>
                  <tr className="border-t border-line bg-surface-soft">
                    <td
                      className="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-ink-faint"
                      colSpan={3}
                    >
                      Разом
                    </td>
                    <td className="nums px-5 py-3 text-right font-semibold text-ink">
                      {formatUAH(total)}
                    </td>
                    <td></td>
                  </tr>
        ...
        <form id="expense-form" onSubmit={add} className="space-y-4">
          ...
          <Field label="Опис" hint="необов’язково">
            <Input
              ...
              placeholder="Оренда за липень"
            />
          </Field>
          {error && (
            <div className="rounded-xl bg-clay-tint px-3.5 py-2.5 text-sm text-clay-ink">
              {error}
            </div>
          )}
        </form>
