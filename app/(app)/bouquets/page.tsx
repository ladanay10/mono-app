...
export default function BouquetsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((b) => {
      ...
      if (q && !(b.title || "без назви").toLowerCase().includes(q))
        return false;
      return true;
    });
  }, [items, query, status]);
  ...
  async function createBouquet() {
    setCreating(true);
    try {
      const b = await api<{ id: string }>("/bouquets", {
        method: "POST",
        body: {},
      });
        ...
        actions={
          <>
            ...
            <Button onClick={createBouquet} disabled={creating}>
              ...
              {creating ? "Створюємо…" : "Новий букет"}
            </Button>
          </>
        }
      />
      ...
      <Card className="overflow-hidden">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            title={
              items.length === 0 ? "Ще немає букетів" : "Нічого не знайдено"
            }
            description={
              items.length === 0
          ...
          <>
            ...
            <ul className="divide-y divide-line lg:hidden">
              {filtered.map((b) => {
                ...
                const pct =
                  gross > 0
                    ? Math.max(0, Math.min(100, (net / gross) * 100))
                    : 0;
                return (
                  <li key={b.id}>
                    <Link
                    ...
                    >
                      ...
                      <div className="shrink-0 text-right">
                        <div className="nums font-semibold text-sage-ink">
                          {formatUAH(net)}
                        </div>
                  ...
                  {filtered.map((b) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="nums px-5 py-3 text-right text-ink-soft">
                          {formatUAH(gross)}
                        </td>
                        <td className="nums px-5 py-3 text-right font-semibold text-sage-ink">
                          {formatUAH(net)}
                        </td>
